...
function generateFallbackJobs(): RawJob[] {
  return [];
}
...
export async function saveJobs(jobs: RawJob[], normalizedRole: string): Promise<number> {
  ...
  for (const job of jobs) {
    try {
      // Guard 1: Normalise title to a predefined canonical role and ensure it matches the search role
      const canonicalTitle = normalizeTitle(job.jobTitle);
      const searchRole = normalizedRole.toLowerCase();
      
      if (!canonicalTitle) {
        console.log(`[Agent] Skipping unrecognised title: "${job.jobTitle}"`);
        continue;
      }

      // Strict match: the job must belong to the role we are searching for
      if (canonicalTitle.toLowerCase() !== searchRole) {
         console.log(`[Agent] Skipping "${job.jobTitle}" (mapped to "${canonicalTitle}") - does not match search role "${searchRole}"`);
         continue;
      }
      
      console.log(`Original title: ${job.jobTitle} → Mapped role: ${canonicalTitle}`);
      ...
      // Step: Also insert a "Direct" link for this job if possible
      // This ensures every platform job has an "Official" alternative in the dashboard
...
export async function runLeadAgent(
  ...
  (async () => {
    try {
      ...
      console.log(
        `[Agent] Collected ${allJobs.length} REAL jobs from ${new Set(allJobs.map((j) => j.companyName)).size} companies`
      );
